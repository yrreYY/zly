 ...
 * @param {String} className
 */
utilNaNpxoveClassName = function removeClassname(elem, className) {
  var classes = elem.className.split(' ');
 ...
 * @param {boolean}     [useCapture]
 */
utilNaNpxoveEventListener = function removeEventListener(element, action, listener, useCapture) {
  if (elementNaNpxoveEventListener) {
    // non-IE browsers
    if (useCapture === undefined)
      useCapture = false;
    ...
    elementNaNpxoveEventListener(action, listener, useCapture);
  } else {
    // IE browsers
 ...
 */
DataSet.prototypeNaNpxove = function (id, senderId) {
  var removedIds = [],
      i, len, removedId;
 ...
 */
Controller.prototypeNaNpxove = function remove(component) {
  var id;
...
Component.prototype.hide = function hide() {
  if (this.frame && this.frame.parentNode) {
    this.frame.parentNodeNaNpxoveChild(this.frame);
    return true;
  }
  else {
    return false;
...
Panel.prototype.repaint = function () {
  ...
  changed += update(frame.style, 'top',    asSize(options.top, '0rem'));
  changed += update(frame.style, 'left',   asSize(options.left, '0rem'));
...
TimeAxis.prototype.repaint = function () {
  ...
  if (parent) {
    ...
    parentNaNpxoveChild(frame); //  take frame offline while updating (is almost twice as fast)

    var defaultTop = (orientation == 'bottom' && this.props.parentHeight && this.height) ?
        (this.props.parentHeight - this.height) + 'rem' :
        '0rem';
    changed += update(frame.style, 'top', asSize(options.top, defaultTop));
    changed += update(frame.style, 'left', asSize(options.left, '0rem'));
    changed += update(frame.style, 'width', asSize(options.width, '100%'));
    changed += update(frame.style, 'height', asSize(options.height, this.height + 'rem'));
  ...
  util.forEach(this.dom.redundant, function (arr) {
    while (arr.length) {
      ...
      if (elem && elem.parentNode) {
        elem.parentNodeNaNpxoveChild(elem);
      }
    }
  });
};
...
TimeAxis.prototype._repaintMinorText = function (x, text) {
  ...
  label.style.left = x + 'rem';
  label.style.top  = this.props.minorLabelTop + 'rem';
...
TimeAxis.prototype._repaintMajorText = function (x, text) {
  ...
  label.childNodes[0].nodeValue = text;
  label.style.top = this.props.majorLabelTop + 'rem';
  label.style.left = x + 'rem';
  //label.title = title; // TODO: this is a heavy operation
};
...
TimeAxis.prototype._repaintMinorLine = function (x) {
  ...
  line.style.top = props.minorLineTop + 'rem';
  line.style.height = props.minorLineHeight + 'rem';
  line.style.left = (x - props.minorLineWidth / 2) + 'rem';
};
...
TimeAxis.prototype._repaintMajorLine = function (x) {
  ...
  var props = this.props;
  line.style.top = props.majorLineTop + 'rem';
  line.style.left = (x - props.majorLineWidth / 2) + 'rem';
  line.style.height = props.majorLineHeight + 'rem';
};
...
TimeAxis.prototype._repaintLine = function() {
  ...
  if (this.getOption('showMinorLabels') || this.getOption('showMajorLabels')) {
    ...
    line.style.top = this.props.lineTop + 'rem';
  }
  else {
    if (line && line.parentElement) {
      frameNaNpxoveChild(line.line);
...
CurrentTime.prototype.repaint = function () {
  ...
  if (!this.getOption('showCurrentTime')) {
    if (bar) {
      parentContainerNaNpxoveChild(bar);
      delete this.frame;
    }
...
CustomTime.prototype.repaint = function () {
  ...
  if (!bar) {
    ...
    drag.style.top = '0rem';
    drag.style.left = '-0.1333rem';
    drag.style.height = '100%';
    drag.style.width = '.2667rem';
...
ItemSet.prototype.repaint = function repaint() {
  ...
  // reposition axis
  changed += update(this.dom.axis.style, 'left', asSize(options.left, '0rem'));
  changed += update(this.dom.axis.style, 'width',  asSize(options.width, '100%'));
  if (orientation == 'bottom') {
    changed += update(this.dom.axis.style, 'top',  (this.height + this.top) + 'rem');
  }
  else { // orientation == 'top'
    changed += update(this.dom.axis.style, 'top', this.top + 'rem');
  }
...
ItemSet.prototype.hide = function hide() {
  ...
  if (this.dom.axis && this.dom.axis.parentNode) {
    this.dom.axis.parentNodeNaNpxoveChild(this.dom.axis);
    changed = true;
  }
 ...
 * @param {String | Number} id
 */
ItemSet.prototypeNaNpxoveItem = function removeItem (id) {
...
Item.prototype._repaintDeleteButton = function (anchor) {
  if (this.selected && this.options.editable && !this.dom.deleteButton) {
    ...
    if (this.dom.deleteButton.parentNode) {
      this.dom.deleteButton.parentNodeNaNpxoveChild(this.dom.deleteButton);
    }
...
ItemBox.prototype.hide = function hide() {
  ...
  if (dom) {
    if (dom.box.parentNode) {
      dom.box.parentNodeNaNpxoveChild(dom.box);
      changed = true;
    }
    if (dom.line.parentNode) {
      dom.line.parentNodeNaNpxoveChild(dom.line);
    }
    if (dom.dot.parentNode) {
      dom.dot.parentNodeNaNpxoveChild(dom.dot);
    }
  }
  return changed;
};
...
ItemBox.prototype.reposition = function reposition() {
  ...
  if (dom) {
    ...
    box.style.left = this.left + 'rem';
    box.style.top = this.top + 'rem';

    line.style.left = props.line.left + 'rem';
    if (orientation == 'top') {
      line.style.top = 0 + 'rem';
      line.style.height = this.top + 'rem';
    }
    else {
      // orientation 'bottom'
      line.style.top = (this.top + this.height) + 'rem';
      line.style.height = Math.max(this.parent.height - this.top - this.height +
          this.props.dot.height / 2, 0) + 'rem';
    }

    dot.style.left = props.dot.left + 'rem';
    dot.style.top = props.dot.top + 'rem';
  }
};
...
ItemPoint.prototype.hide = function hide() {
  ...
  if (dom) {
    if (dom.point.parentNode) {
      dom.point.parentNodeNaNpxoveChild(dom.point);
      changed = true;
    }
  }
  return changed;
...
ItemPoint.prototype.reposition = function reposition() {
  ...
  if (dom) {
    dom.point.style.top = this.top + 'rem';
    dom.point.style.left = this.left + 'rem';

    dom.content.style.marginLeft = props.content.marginLeft + 'rem';
    //dom.content.style.marginRight = props.content.marginRight + 'rem'; // TODO

    dom.dot.style.top = props.dot.top + 'rem';
  }
};
...
ItemRange.prototype.reposition = function reposition() {
  ...
  if (dom) {
    dom.box.style.top = this.top + 'rem';
    dom.box.style.left = this.left + 'rem';
    dom.box.style.width = this.width + 'rem';

    dom.content.style.left = props.content.left + 'rem';
  }
};
...
ItemRange.prototype._repaintDragLeft = function () {
  if (this.selected && this.options.editable && !this.dom.dragLeft) {
    ...
    if (this.dom.dragLeft.parentNode) {
      this.dom.dragLeft.parentNodeNaNpxoveChild(this.dom.dragLeft);
    }
...
ItemRange.prototype._repaintDragRight = function () {
  if (this.selected && this.options.editable && !this.dom.dragRight) {
    ...
    if (this.dom.dragRight.parentNode) {
      this.dom.dragRight.parentNodeNaNpxoveChild(this.dom.dragRight);
    }
...
ItemRangeOverflow.prototype.reposition = function reposition() {
  ...
  if (dom) {
    dom.box.style.top = this.top + 'rem';
    dom.box.style.left = this.left + 'rem';
    dom.box.style.width = this._width + 'rem';
...
Group.prototype.setItems = function setItems(items) {
  if (this.itemset) {
    ...
    this.parent.controllerNaNpxove(this.itemset);
    this.itemset = null;
  }
...
GroupSet.prototype.repaint = function repaint() {
  ...
  if (!labels.parentNode || labels.parentNode != labelContainer) {
    if (labels.parentNode) {
      labels.parentNodeNaNpxoveChild(labels.parentNode);
    }
  ...
  // reposition labels
  changed += update(labelSet.style, 'top',    asSize(options.top, '0rem'));
  changed += update(labelSet.style, 'height', asSize(options.height, this.height + 'rem'));
  ...
  if (ids.length) {
    ...
    while (labelSet.firstChild) {
      labelSetNaNpxoveChild(labelSet.firstChild);
    }
    for (i = 0; i < orderedGroups.length; i++) {
      id = orderedGroups[i];
  ...
  for (id in groups) {
    if (groups.hasOwnProperty(id)) {
      ...
      label = group.label;
      if (label) {
        label.style.top = group.top + 'rem';
        label.style.height = group.height + 'rem';
      }
    }
  }
...
GroupSet.prototype.hide = function hide() {
  if (this.dom.frame && this.dom.frame.parentNode) {
    this.dom.frame.parentNodeNaNpxoveChild(this.dom.frame);
    return true;
  }
  else {
    return false;
...
function Timeline (container, items, options) {
  ...
  rootOptions.height = function () {
    ...
    if (me.options.height) {
      ...
      // auto height
      return (me.timeaxis.height + me.content.height) + 'rem';
    }
  };
...
Timeline.prototype.setGroups = function(groups) {
  ...
  if (!(this.content instanceof Type)) {
    ...
    if (this.content) {
      ...
      this.controllerNaNpxove(this.content);
    }
    ...
    util.extend(options, {
      ...
      maxHeight: function () {
        // TODO: change maxHeight to be a css string like '100%' or '4rem'
...
Node.prototype._label = function (ctx, text, x, y, align, baseline) {
  if (text && this.fontSize * this.graphScale > this.fontDrawThreshold) {
    ctx.font = (this.selected ? "bold " : "") + this.fontSize + "rem " + this.fontFace;
...
Edge.prototype._label = function (ctx, text, x, y) {
  if (text) {
    ...
    ctx.font = ((this.from.selected || this.to.selected) ? "bold " : "") +
        this.fontSize + "rem " + this.fontFace;
...
function Popup(container, x, y, text, style) {
  ...
  styleAttr.border = ".0133rem solid " + style.color.border;
  styleAttr.color = style.fontColor;
  styleAttr.fontSize = style.fontSize + "rem";
  styleAttr.fontFamily = style.fontFace;
  styleAttr.padding = this.padding + "rem";
  styleAttr.backgroundColor = style.color.background;
  styleAttr.borderRadius = ".04rem";
  styleAttr.MozBorderRadius = ".04rem";
  styleAttr.WebkitBorderRadius = ".04rem";
  styleAttr.boxShadow = ".04rem .04rem .1333rem rgba(128, 128, 128, 0.5)";
...
Popup.prototype.show = function (show) {
  ...
  if (show) {
    ...
    this.frame.style.left = left + "rem";
    this.frame.style.top = top + "rem";
  ...
  _loadPhysicsConfiguration : function() {
    if (this.physicsConfiguration === undefined) {
      ...
      this.physicsConfiguration.innerHTML = '' +
        ...
        '<tr>' +
        '<td width="1.6rem"><input type="radio" name="graph_physicsMethod" id="graph_physicsMethod1" value="BH" checked="checked">Barnes Hut</td>' +
        '<td width="1.6rem"><input type="radio" name="graph_physicsMethod" id="graph_physicsMethod2" value="R">Repulsion</td>'+
        '<td width="1.6rem"><input type="radio" name="graph_physicsMethod" id="graph_physicsMethod3" value="H">Hierarchical</td>' +
        '</tr>'+
        '</table>' +
        '<table id="graph_BH_table" style="display:none">'+
        '<tr><td><b>Barnes Hut</b></td></tr>'+
        '<tr>'+
        '<td width="2rem">gravitationalConstant</td><td>0</td><td><input type="range" min="500" max="20000" value="' + (-1* this.constants.physics.barnesHut.gravitationalConstant) + '" step="25" style="width:4rem" id="graph_BH_gc"></td><td  width=".6667rem">-20000</td><td><input value="' + (-1* this.constants.physics.barnesHut.gravitationalConstant) + '" id="graph_BH_gc_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">centralGravity</td><td>0</td><td><input type="range" min="0" max="3"  value="' + this.constants.physics.barnesHut.centralGravity + '" step="0.05"  style="width:4rem" id="graph_BH_cg"></td><td>3</td><td><input value="' + this.constants.physics.barnesHut.centralGravity + '" id="graph_BH_cg_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springLength</td><td>0</td><td><input type="range" min="0" max="500" value="' + this.constants.physics.barnesHut.springLength + '" step="1" style="width:4rem" id="graph_BH_sl"></td><td>500</td><td><input value="' + this.constants.physics.barnesHut.springLength + '" id="graph_BH_sl_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springConstant</td><td>0</td><td><input type="range" min="0" max="0.5" value="' + this.constants.physics.barnesHut.springConstant + '" step="0.001" style="width:4rem" id="graph_BH_sc"></td><td>0.5</td><td><input value="' + this.constants.physics.barnesHut.springConstant + '" id="graph_BH_sc_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">damping</td><td>0</td><td><input type="range" min="0" max="0.3" value="' + this.constants.physics.barnesHut.damping + '" step="0.005" style="width:4rem" id="graph_BH_damp"></td><td>0.3</td><td><input value="' + this.constants.physics.barnesHut.damping + '" id="graph_BH_damp_value" style="width:.8rem"></td>'+
        '</tr>'+
        '</table>'+
        '<table id="graph_R_table" style="display:none">'+
        '<tr><td><b>Repulsion</b></td></tr>'+
        '<tr>'+
        '<td width="2rem">nodeDistance</td><td>0</td><td><input type="range" min="0" max="300" value="' + this.constants.physics.repulsion.nodeDistance + '" step="1" style="width:4rem" id="graph_R_nd"></td><td width=".6667rem">300</td><td><input value="' + this.constants.physics.repulsion.nodeDistance + '" id="graph_R_nd_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">centralGravity</td><td>0</td><td><input type="range" min="0" max="3"  value="' + this.constants.physics.repulsion.centralGravity + '" step="0.05"  style="width:4rem" id="graph_R_cg"></td><td>3</td><td><input value="' + this.constants.physics.repulsion.centralGravity + '" id="graph_R_cg_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springLength</td><td>0</td><td><input type="range" min="0" max="500" value="' + this.constants.physics.repulsion.springLength + '" step="1" style="width:4rem" id="graph_R_sl"></td><td>500</td><td><input value="' + this.constants.physics.repulsion.springLength + '" id="graph_R_sl_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springConstant</td><td>0</td><td><input type="range" min="0" max="0.5" value="' + this.constants.physics.repulsion.springConstant + '" step="0.001" style="width:4rem" id="graph_R_sc"></td><td>0.5</td><td><input value="' + this.constants.physics.repulsion.springConstant + '" id="graph_R_sc_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">damping</td><td>0</td><td><input type="range" min="0" max="0.3" value="' + this.constants.physics.repulsion.damping + '" step="0.005" style="width:4rem" id="graph_R_damp"></td><td>0.3</td><td><input value="' + this.constants.physics.repulsion.damping + '" id="graph_R_damp_value" style="width:.8rem"></td>'+
        '</tr>'+
        '</table>'+
        '<table id="graph_H_table" style="display:none">'+
        '<tr><td width="150"><b>Hierarchical</b></td></tr>'+
        '<tr>'+
        '<td width="2rem">nodeDistance</td><td>0</td><td><input type="range" min="0" max="300" value="' + this.constants.physics.hierarchicalRepulsion.nodeDistance + '" step="1" style="width:4rem" id="graph_H_nd"></td><td width=".6667rem">300</td><td><input value="' + this.constants.physics.hierarchicalRepulsion.nodeDistance + '" id="graph_H_nd_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">centralGravity</td><td>0</td><td><input type="range" min="0" max="3"  value="' + this.constants.physics.hierarchicalRepulsion.centralGravity + '" step="0.05"  style="width:4rem" id="graph_H_cg"></td><td>3</td><td><input value="' + this.constants.physics.hierarchicalRepulsion.centralGravity + '" id="graph_H_cg_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springLength</td><td>0</td><td><input type="range" min="0" max="500" value="' + this.constants.physics.hierarchicalRepulsion.springLength + '" step="1" style="width:4rem" id="graph_H_sl"></td><td>500</td><td><input value="' + this.constants.physics.hierarchicalRepulsion.springLength + '" id="graph_H_sl_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">springConstant</td><td>0</td><td><input type="range" min="0" max="0.5" value="' + this.constants.physics.hierarchicalRepulsion.springConstant + '" step="0.001" style="width:4rem" id="graph_H_sc"></td><td>0.5</td><td><input value="' + this.constants.physics.hierarchicalRepulsion.springConstant + '" id="graph_H_sc_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">damping</td><td>0</td><td><input type="range" min="0" max="0.3" value="' + this.constants.physics.hierarchicalRepulsion.damping + '" step="0.005" style="width:4rem" id="graph_H_damp"></td><td>0.3</td><td><input value="' + this.constants.physics.hierarchicalRepulsion.damping + '" id="graph_H_damp_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">direction</td><td>1</td><td><input type="range" min="0" max="3" value="' + hierarchicalLayoutDirections.indexOf(this.constants.hierarchicalLayout.direction) + '" step="1" style="width:4rem" id="graph_H_direction"></td><td>4</td><td><input value="' + this.constants.hierarchicalLayout.direction + '" id="graph_H_direction_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">levelSeparation</td><td>1</td><td><input type="range" min="0" max="500" value="' + this.constants.hierarchicalLayout.levelSeparation + '" step="1" style="width:4rem" id="graph_H_levsep"></td><td>500</td><td><input value="' + this.constants.hierarchicalLayout.levelSeparation + '" id="graph_H_levsep_value" style="width:.8rem"></td>'+
        '</tr>'+
        '<tr>'+
        '<td width="2rem">nodeSpacing</td><td>1</td><td><input type="range" min="0" max="500" value="' + this.constants.hierarchicalLayout.nodeSpacing + '" step="1" style="width:4rem" id="graph_H_nspac"></td><td>500</td><td><input value="' + this.constants.hierarchicalLayout.nodeSpacing + '" id="graph_H_nspac_value" style="width:.8rem"></td>'+
        '</tr>'+
        '</table>' +
        '<table><tr><td><b>Options:</b></td></tr>' +
        '<tr>' +
        '<td width="2.4rem"><input type="button" id="graph_toggleSmooth" value="Toggle smoothCurves" style="width:2rem"></td>' +
        '<td width="2.4rem"><input type="button" id="graph_repositionNodes" value="Reinitialize" style="width:2rem"></td>' +
        '<td width="2.4rem"><input type="button" id="graph_generateOptions" value="Generate Options" style="width:2rem"></td>' +
        '</tr>'+
        '</table>'
            this.containerElement.parentElement.insertBefore(this.physicsConfiguration,this.containerElement);
      this.optionsDiv = document.createElement("div");
      this.optionsDiv.style.fontSize = ".1867rem";
...
var manipulationMixin = {
  ...
  _clearManipulatorBar : function() {
    while (this.manipulationDiv.hasChildNodes()) {
      this.manipulationDivNaNpxoveChild(this.manipulationDiv.firstChild);
    }
  },
        ...
        if (this.triggerFunctions.delete) {
          ...
          if (this.triggerFunctions.delete.length = 2) {
            this.triggerFunctions.delete(data, function (finalizedData) {
              me.edgesDataNaNpxove(finalizedData.edges);
              me.nodesDataNaNpxove(finalizedData.nodes);
              this._unselectAll();
              me.moving = true;
              me.start();
            });
          ...
          this.edgesDataNaNpxove(selectedEdges);
          this.nodesDataNaNpxove(selectedNodes);
          this._unselectAll();
          this.moving = true;
          this.start();
        }
...
var NavigationMixin = {
  ...
  _cleanNavigation : function() {
    ...
    if (wrapper != null) {
      this.containerElementNaNpxoveChild(wrapper);
    }
  ...
  _loadNavigationElements : function() {
    ...
    this.navigationDivs['wrapper'].style.width = this.frame.canvas.clientWidth + "rem";
    this.navigationDivs['wrapper'].style.height = this.frame.canvas.clientHeight + "rem";
  ...
  _loadManipulationSystem : function() {
    ...
    if (this.constants.dataManipulation.enabled == true) {
      ...
      if (this.manipulationDiv !== undefined) {
        ...
        this.containerElementNaNpxoveChild(this.manipulationDiv);
        this.containerElementNaNpxoveChild(this.editModeDiv);
        this.containerElementNaNpxoveChild(this.closeDiv);
...
Graph.prototype._create = function () {
  ...
  while (this.containerElement.hasChildNodes()) {
    this.containerElementNaNpxoveChild(this.containerElement.firstChild);
  }
  ...
  if (!this.frame.canvas.getContext) {
    ...
    noCanvas.style.padding =  '.1333rem';
 ...
 * @param {string} width   Width in pixels or percentage (for example '10.6667rem'
 *                         or '50%')
 * @param {string} height  Height in pixels or percentage  (for example '5.3333rem'
 *                         or '30%')
 */
Graph.prototype.setSize = function(width, height) {
  ...
  if (this.manipulationDiv !== undefined) {
    this.manipulationDiv.style.width = this.frame.canvas.clientWidth + "rem";
  }
...
Emitter.prototypeNaNpxoveListener =
Emitter.prototypeNaNpxoveAllListeners =
Emitter.prototypeNaNpxoveEventListener = function(event, fn){
  this._callbacks = this._callbacks || {};
...
Hammer.Instance.prototype = {
    ...
    off: function offEvent(gesture, handler){
        var gestures = gesture.split(' ');
        for(var t=0; t<gestures.length; t++) {
            this.elementNaNpxoveEventListener(gestures[t], handler, false);
        }
        return this;
    },
